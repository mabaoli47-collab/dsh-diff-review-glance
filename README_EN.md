...
**Threat model**: the plugin's trust boundary is the local loopback plus the host's listening address. The plugin **never uploads any data** — all communication is local browser ↔ host traffic. The realtime preview bucket (live) is workspace-level data: reads/actions on it are not bound to the initiating session, but writes (revert) only happen from a same-origin local client and are gated by `liveRevert` plus version-conflict protection.
