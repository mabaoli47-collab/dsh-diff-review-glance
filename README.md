...
- **可选实时预览（v0.5）**：检测模式可在「回合结束」与「实时预览」间切换——实时模式用 `fs.watch` 监听工作区（仅 Windows），对话进行中即显示只读「进行中修改」预览，回合结束后并入正式审阅项（避免抓到写文件的中间态）
...
3. 可选：在 设置 → Diff 审阅插件 将检测模式切换为「实时预览」，对话进行中即可在 dock 顶部看到只读的「进行中修改（实时预览）」块（仅 Windows）
...
- **检测时机**：`agent/turn-stopping`（回合结束）触发全量 `walkWorkspace` 版本对比（默认，跨平台）；`detectMode=live` 时额外用 `fs.watch` 递归监听工作区，事件去抖 600ms 后做**单文件/全量增量比对**（watcher 只当触发器，正确性仍以版本对比为准，回合末全量扫描保留为兜底——watcher 丢事件只会延迟不会漏检），实时变更挂 `store.live` 只读预览桶，回合结束正式扫描后清空并入正式审阅项
