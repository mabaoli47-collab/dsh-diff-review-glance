...
- **可选实时预览（v0.5）**：检测模式可在「回合结束」与「实时预览」间切换——实时模式用 `fs.watch` 监听工作区（仅 Windows），对话进行中即显示「进行中修改」预览，**可直接展开 diff 或外部打开**；实时撤销默认关闭（设置项 `liveRevert`，开启后带版本冲突保护）；回合结束后并入正式审阅项（避免抓到写文件的中间态）
...
- **实时预览（live 模式）的资源与可靠性**：`detectMode=live` 会对每个访问过的工作区维持一个递归 `fs.watch` 句柄（Windows，多工作区切换时句柄累积，随插件卸载释放）；watcher 事件仅作触发信号——去抖窗口内的文件名会累积成集合逐文件比对，目录级事件或路径过多时转全量，另有 8 秒事件静默自动全量兜底，因此丢事件不会漏检、只会延迟；文件变动频繁的工作区会带来周期性增量比对开销。该模式仅 Windows 实现，非 Windows 自动回退回合模式。
