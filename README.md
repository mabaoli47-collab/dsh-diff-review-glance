...
- **可选实时预览（v0.5）**：检测模式可在「回合结束」与「实时预览」间切换——实时模式用 `fs.watch` 监听工作区（仅 Windows），对话进行中即显示「进行中修改」预览，**可直接展开 diff、撤销（带版本冲突保护）或外部打开**；回合结束后并入正式审阅项（避免抓到写文件的中间态）
...
3. 可选：在 设置 → Diff 审阅插件 将检测模式切换为「实时预览」，对话进行中即可在 dock 顶部看到「进行中修改（实时预览）」块（仅 Windows）——可直接展开 diff、撤销（带版本冲突保护）或外部打开，无需等回合结束
