...
- **读取当前工作区文件内容**：为建立文件版本基线并检测修改，插件会遍历并预读工作区文本（默认排除 `.env*`、`*.pem` / `*.key` / `*.p12` / `*.pfx`、`credentials.*`、`secrets.*`、`config.local.*` 等敏感文件，不纳入对比与缓存）；
...
- **本地 HTTP API**：`/dsh-diff-review` 仅限本机回环地址访问；写操作校验请求来源（防浏览器 CSRF 与 DNS 重绑定）。

插件**不会**将任何数据上传网络——所有通信均为浏览器 ↔ 本机宿主之间的本地请求。
